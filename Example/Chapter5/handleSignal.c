#include <stdio.h>
#include <signal.h>                                       /* signal( ) 함수를 위해서 사용 */
#include <stdlib.h>                                       /* exit( ) 함수를 위해서 사용 */
#include <unistd.h>
#include <sys/types.h>
...
int main(int argc, char** argv)
{
  ...
  sigprocmask(SIG_BLOCK, &pset, NULL);                    /* 블록으로 설정 */
...
{
  if(signo == SIGINT) {                                          /* SIGINT 시그널이 발생했을때 처리 */
    printf("SIGINT is catched : %d\n", signo);
    exit(0);
  } else if(signo == SIGUSR1) {                             /* SIGUSR1 시그널이 발생했을때 처리 */
    printf("SIGUSR1 is catched\n");
  } else if(signo == SIGUSR2) {                            /* SIGUSR2 시그널이 발생했을때 처리 */
...
{
  int i;
  for(i = 1; i < NSIG; ++i) {                                /* sigset_t에 설정된 전체 비트를 출력한다. */

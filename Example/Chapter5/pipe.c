#include <stdio.h>                      /* printf( ) 함수를 위해서 사용 */
#include <unistd.h>
#include <sys/types.h>
...
int main(int argc, char** argv)
{
  ...
  if((pid = fork()) < 0) {                /* fork( ) 함수를 이용해서 프로세스 생성 */
     ...
     close(pfd[0]);                          /* 쓰기를 위한 파일 디스크립터 닫기 */
     _exit(127);
  } else {                                       /* 부모 프로세스인 경우의 처리 */
     close(pfd[1]);                          /* 쓰기를 위한 파일 디스크립터 닫기 */
     if(read(pfd[0], line, BUFSIZ) < 0) {      /* 파일 디스크립터로 부터 데이터 읽기 */
        perror("read");
        return -1;
     }
     printf("%s", line);                    /* 파일 디스크립터로 부터 읽은 내용을 화면에 표시 */
